                  ...
                  <div class="col-8">
                    ...
                    <nav aria-label="breadcrumb">
                      <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">กลุ่มบุคลากร</li>
                      </ol>
                    </nav>
                  </div>
                  <div class="col-4 text-right pt-4">
                    <button class="btn btn-primary" type="button" onclick="window.location='app-page-create'"><i class="bx bx-plus"></i> สร้างเนื้อหาใหม่</button>
                  </div>
                  <div class="col-12">
                    <div class="card">
                      ...
                      <div class="card-body">
                        <table class="table table-border-top" id="table-1">
                          ...
                          <tbody>
                            <?php 
                            $strSQL = "SELECT * FROM ecw_person_group WHERE cat_status = 'Y' ORDER BY cat_id ASC";
                            ...
                            if(($res) && ($res['status'])){
                              ...
                              foreach ($res['data'] as $row) {
                                ...
                                <tr id="tbrow_<?php echo $c;?>" >
                                  <td class="text-top" style="width: 40px;" ><?php echo $c ; ?> </td>
                                  <td class="text-top" >
                                    <?php echo $row['cat_name'] ; ?>
                                  </td>
                                  <td class="text-top" style="width: 150px;">
                                  <?php 
                                  $strSQL = "SELECT COUNT(1) cm FROM ecw_person WHERE per_group_id = '".$row['cat_id']."'";
                                  $res = $db->fetch($strSQL, false, false);
                                  if($res){
                                    echo number_format($res['cm'], 0, '.', ',');
                                  }else{
                                    echo "0";
                                  }
                                  ?>
                                  </td>
                                  <td class="text-top"  style="text-align: right;">
                                      <a href="app-page-edit?id=<?php echo $row['cat_id'];?>" class="btn btn-icon"><i class="bx bx-pencil"></i></a>
                                      <a href="Javascript:content.delete('<?php echo $row['cat_id'];?>', '<?php echo $c; ?>')" class="btn btn-icon"><i class="bx bx-trash text-danger"></i></a>
                                  </td>
                                </tr>
                                <?php
                                $c++;
