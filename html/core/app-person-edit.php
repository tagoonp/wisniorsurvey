...
if(!isset($_REQUEST['id'])){
  $db->close($conn);
  header('Location: app-person-list');
  die();
}

$content_id = mysqli_real_escape_string($conn, $_REQUEST['id']);
$strSQL = "SELECT * FROM ecw_person WHERE per_id = '$content_id'";
                  ...
                  <div class="col-12">
                    ...
                    <nav aria-label="breadcrumb">
                      <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                          <a href="app-person-list">บุคลากร</a>
                        </li>
                      ...
                      <div class="card-body">
                        <div class="row">
                          <div class="form-group col-12">
                            ...
                            <input type="text" name="txtFullname" id="txtFullname" class="form-control" value="<?php echo $resContent['per_name']; ?>">
                          </div>

                          <div class="form-group col-12">
                            <label for="">ตำแหน่ง : </label>
                            <input type="text" name="txtPosittion" id="txtPosition" class="form-control" value="<?php echo $resContent['per_subtitle']; ?>">
                          </div>
                          ...
                          <div class="form-group col-12">
                            ...
                            <select name="txtCategory" id="txtCategory" class="form-control">
                              ...
                              <?php 
                              $strSQL = "SELECT * FROM ecw_person_group WHERE cat_status = 'Y' ORDER BY cat_id";
                              $res = $db->fetch($strSQL, true, true);
                              if(($res) && ($res['status'])){
                                foreach ($res['data'] as $row) {
                                  ?>
                                  <option value="<?php echo $row['cat_id'];?>" <?php if($resContent['per_group_id'] == $row['cat_id']){ echo "selected"; }?>><?php echo $row['cat_name'];?></option>
                                  <?php
                                }
                              }
                              ?>
                          ...
                          <div class="form-group col-12" style="height: 280px;">
                            <label for="">คำอธิบาย/รายละเอียด/ข้อมูล : <span class="text-danger">*</span></label>
                            <!-- <textarea name="txtContent" id="txtContent" cols="30" rows="10" class="form-control"></textarea> -->
                            <div id="full-editor" style="height: 200px;"><?php echo $resContent['per_info']; ?></div>
                          </div>

                          <div class="form-group col-12">
                            <label for="">Figure image : </label>
                            <input type="text" name="txtImage" id="txtImage" class="form-control"  value="<?php echo $resContent['per_img']; ?>">
                          ...
                          <div class="form-group col-12 text-right">
                            <button class="btn btn-primary" type="button" onclick="content.update_person()">บันทึก</button>
                          </div>
